\documentclass[12pt, a4paper, openany]{book}

% --- Core Packages ---
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[margin=1.2in, bindingoffset=0.2in]{geometry} % Professional Thesis Margins
\usepackage{setspace, adjustbox, float, subfig, array, multirow, booktabs, appendix}
\usepackage{graphicx}
\usepackage[pdfborder={0 0 0}, colorlinks=true, linkcolor=black, citecolor=blue]{hyperref}
\usepackage{algorithmic, algorithm, listings}
\usepackage{etoolbox}
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows, positioning}
\usepackage{fancyhdr} % Better control over headers and footers
\usepackage{enumitem} % Professional list spacing
\usepackage{titlesec} % Professional heading spacing
\usepackage{graphicx}
\usepackage{float} % Helps with precise image placement [H]

% --- Professional Spacing Rules ---
\setlist{itemsep=0.2em, topsep=0.4em, parsep=0.1em} % Compact lists
\titleformat{\chapter}[display]{\normalfont\huge\bfseries}{\chaptertitlename\ \thechapter}{20pt}{\Huge}
\titlespacing*{\chapter}{0pt}{-30pt}{20pt} % Reduce huge gaps above chapters
\titlespacing*{\section}{0pt}{12pt}{6pt} % Control section spacing
\titlespacing*{\subsection}{0pt}{10pt}{4pt}

% --- Page Layout & Headers ---
\pagestyle{fancy}
\fancyhf{} % Clear all header and footer fields
\fancyhead[RO,LE]{\small\nouppercase{\leftmark}} % Right-Odd, Left-Even headers
\fancyfoot[C]{\thepage} % Center footer page number
\renewcommand{\headrulewidth}{0.4pt} % Thin header line
\setlength{\headheight}{15pt} % Fix header height warning
\fancypagestyle{plain}{ % Re-define plain style (chapter pages)
  \fancyhf{}
  \fancyfoot[C]{\thepage}
  \renewcommand{\headrulewidth}{0pt}
}

% --- NEW: Call Metadata File ---
\input{metadata} % This pulls in all your variables

% --- Lean 4 Listings Configuration ---
\lstdefinelanguage{lean}{
    keywords={theorem, lemma, definition, def, tactic, by, intro, exact},
    sensitive=true,
    comment=[l]{--}
}
\lstset{
    language=lean,
    extendedchars=true,
    literate={α}{{\ensuremath{\alpha}}}1 {∩}{{\ensuremath{\cap}}}1 {⊆}{{\ensuremath{\subseteq}}}1 {∈}{{\ensuremath{\in}}}1 {∨}{{\ensuremath{\lor}}}1 {∧}{{\ensuremath{\land}}}1 {∅}{{\ensuremath{\emptyset}}}1 {→}{{\ensuremath{\rightarrow}}}1 {↔}{{\ensuremath{\leftrightarrow}}}1 {∀}{{\ensuremath{\forall}}}1 {∃}{{\ensuremath{\exists}}}1 {×}{{\ensuremath{\times}}}1 {₁}{{\ensuremath{_1}}}1 {₂}{{\ensuremath{_2}}}1,
    basicstyle=\ttfamily\small,
    inputencoding=utf8
}

\begin{document}
\onehalfspacing

\pagestyle{plain}
\pagenumbering{Roman}

% --- Front Matter ---
% IMPORTANT: Rename "99GUC_TitlePage.tex.tex" to "99GUC_TitlePage.tex" in your sidebar
\include{99GUC_TitlePage}
\include{99Abstract}
\include{99Acknowledgments}

\tableofcontents
\listoffigures
\clearpage 

% --- Main Body ---
\pagestyle{fancy}
\pagenumbering{arabic}
\setlength\parskip{0.3em} % Professional smaller paragraph spacing
\setlength\parindent{2em} % Standard indent for first lines

% These files must exist in your sidebar with these exact names
\include{01Introduction}
\include{02Background}
\include{03Methodology}
\include{04Results}
\include{05Conclusion}

% --- Back Matter ---
% Ensure your file is named "99BScBiblio.bib"
\bibliography{99BScBiblio}
\bibliographystyle{ieeetr}
\addcontentsline{toc}{chapter}{References}

\end{document}